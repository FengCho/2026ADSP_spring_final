...
BME_GROUP_LABELS = ["g1", "g2", "g3", "g4", "g5", "g6"]
...
def plot_psd_example(
    ...
    for subject in sorted(psd_examples.keys()):
        for class_id in sorted(psd_examples[subject].keys()):
            ...
            ax.set_title(f"{subject} {class_name} seg{seg_id}")
            ax.set_xlim(0, 40)
...
def plot_bme_comparison(summary: dict, windows_sec: list[int], out_path: Path) -> None:
    ...
    ax.set_title("Six BME Course Group Window Choices vs This Study")
    ax.legend()
    fig.tight_layout()
    fig.savefig(out_path, format="pdf")
    plt.close(fig)
